<template>
  <div class="transition-all duration-300" :class="sidebarCollapsed ? 'sm:ml-16' : 'sm:ml-80'">
    <div class="p-6 mt-14 min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <!-- PJ Section Excel Upload -->
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">PJ Section Excel Upload</h3>
        </div>
        <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center">
          ...
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Upload .xlsx or .xls files with PJ Section data (RACK No., Size, Balance stock, Rate, Value)
          </p>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-40 flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded p-4 w-full max-w-lg z-50">
          <h3 class="font-semibold mb-2">Create PJ Section Product</h3>
...
const visibleProducts = computed(() => {
  // Filter only PJ Section products
  let list = products.value.filter(p => p.category === 'PJ Section')
...
// Parse PJ Section Excel row (array-based: [Rack, Size, Stock, Rate, Value])
const parseASectionExcelRow = (row: any[], idx: number): Product | null => {
  const PJ = row?.[0]
  const RIBS = row?.[1]

  // Skip empty rows
  if (!PJ && !RIBS) return null
  if (!RIBS) return null
  ...
  return {
    ...
    section: PJ !== undefined && PJ !== null && PJ !== '' ? String(PJ) : 'Unknown',
    category: 'PJ Section',
    name: `${PJ ? String(PJ) : 'PJ'}-${meter}`,
    size: String(RIBS),
    sku: `${PJ ? String(PJ) : 'PJ'}-${RIBS}`,
    stock,
    rate,
...
// Handle PJ Section Excel file upload
const handleASectionExcelUpload = async (event: Event) => {
  ...
  try {
    // Always clear existing PJ Section local data and history before processing
    products.value = products.value.filter(p => p.category !== 'PJ Section')
    persistProducts()
    ...
    dataRows.forEach((row: any[], idx: number) => {
      ...
      if (!product) {
        ...
        console.warn('PJ Section upload skipped row', { idx, row })
        return
      }
      ...
      if (product.stock > 0) {
        addTransaction({
          ...
          timestamp: Date.now(),
          type: 'IN',
          description: `Excel upload: New PJ Section product added`,
          stockBefore: 0,
...
const createProduct = () => {
  ...
  const p: Product = { 
    id, 
    category: 'PJ Section',
    section: c.section || 'PJ',
    name: c.name || `PJ-${c.size}`,
    sku: `${c.section || 'PJ'}-${c.size}`,
    size: c.size || '',
