<template>
  <div class="transition-all duration-300" :class="sidebarCollapsed ? 'sm:ml-16' : 'sm:ml-80'">
    <div class="p-6 mt-14 min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <!-- DPL Section Excel Upload -->
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">DPL Section Excel Upload</h3>
        </div>
        <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center">
          ...
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Upload .xlsx or .xls files with DPL Section data (RACK No., Size, Balance stock, Rate, Value)
          </p>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-40 flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded p-4 w-full max-w-lg z-50">
          <h3 class="font-semibold mb-2">Create DPL Section Product</h3>
...
const visibleProducts = computed(() => {
  // Filter only DPL Section products
  let list = products.value.filter(p => p.category === 'DPL Section')
...
// Parse DPL Section Excel row (array-based: [Rack, Size, Stock, Rate, Value])
const parseASectionExcelRow = (row: any[], idx: number): Product | null => {
  const DPL = row?.[0]
  const RIBS = row?.[1]

  // Skip empty rows
  if (!DPL && !RIBS) return null
  if (!RIBS) return null
  ...
  return {
    ...
    section: DPL !== undefined && DPL !== null && DPL !== '' ? String(DPL) : 'Unknown',
    category: 'DPL Section',
    name: `${DPL ? String(DPL) : 'DPL'}-${meter}`,
    size: String(RIBS),
    sku: `${DPL ? String(DPL) : 'DPL'}-${RIBS}`,
    stock,
    rate,
...
// Handle DPL Section Excel file upload
const handleASectionExcelUpload = async (event: Event) => {
  ...
  try {
    // Always clear existing DPL Section local data and history before processing
    products.value = products.value.filter(p => p.category !== 'DPL Section')
    persistProducts()
    ...
    dataRows.forEach((row: any[], idx: number) => {
      ...
      if (!product) {
        ...
        console.warn('DPL Section upload skipped row', { idx, row })
        return
      }
      ...
      if (product.stock > 0) {
        addTransaction({
          ...
          timestamp: Date.now(),
          type: 'IN',
          description: `Excel upload: New DPL Section product added`,
          stockBefore: 0,
...
const createProduct = () => {
  ...
  const p: Product = { 
    id, 
    category: 'DPL Section',
    section: c.section || 'DPL',
    name: c.name || `DPL-${c.size}`,
    sku: `${c.section || 'DPL'}-${c.size}`,
    size: c.size || '',
