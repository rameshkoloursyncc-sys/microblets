<template>
  <div class="transition-all duration-300" :class="sidebarCollapsed ? 'sm:ml-16' : 'sm:ml-80'">
    <div class="p-6 mt-14 min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <!-- PL Section Excel Upload -->
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">PL Section Excel Upload</h3>
        </div>
        <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center">
          ...
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Upload .xlsx or .xls files with PL Section data (RACK No., Size, Balance stock, Rate, Value)
          </p>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-40 flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded p-4 w-full max-w-lg z-50">
          <h3 class="font-semibold mb-2">Create PL Section Product</h3>
...
const visibleProducts = computed(() => {
  // Filter only PL Section products
  let list = products.value.filter(p => p.category === 'PL Section')
...
// Parse PL Section Excel row (array-based: [Rack, Size, Stock, Rate, Value])
const parseASectionExcelRow = (row: any[], idx: number): Product | null => {
  const PL = row?.[0]
  const RIBS = row?.[1]

  // Skip empty rows
  if (!PL && !RIBS) return null
  if (!RIBS) return null
  ...
  return {
    ...
    section: PL !== undefined && PL !== null && PL !== '' ? String(PL) : 'Unknown',
    category: 'PL Section',
    name: `${PL ? String(PL) : 'PL'}-${meter}`,
    size: String(RIBS),
    sku: `${PL ? String(PL) : 'PL'}-${RIBS}`,
    stock,
    rate,
...
// Handle PL Section Excel file upload
const handleASectionExcelUpload = async (event: Event) => {
  ...
  try {
    // Always clear existing PL Section local data and history before processing
    products.value = products.value.filter(p => p.category !== 'PL Section')
    persistProducts()
    ...
    dataRows.forEach((row: any[], idx: number) => {
      ...
      if (!product) {
        ...
        console.warn('PL Section upload skipped row', { idx, row })
        return
      }
      ...
      if (product.stock > 0) {
        addTransaction({
          ...
          timestamp: Date.now(),
          type: 'IN',
          description: `Excel upload: New PL Section product added`,
          stockBefore: 0,
...
const createProduct = () => {
  ...
  const p: Product = { 
    id, 
    category: 'PL Section',
    section: c.section || 'PL',
    name: c.name || `PL-${c.size}`,
    sku: `${c.section || 'PL'}-${c.size}`,
    size: c.size || '',
