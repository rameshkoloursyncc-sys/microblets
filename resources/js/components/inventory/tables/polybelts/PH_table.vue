<template>
  <div class="transition-all duration-300" :class="sidebarCollapsed ? 'sm:ml-16' : 'sm:ml-80'">
    <div class="p-6 mt-14 min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <!-- PH Section Excel Upload -->
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">PH Section Excel Upload</h3>
        </div>
        <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center">
          ...
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Upload .xlsx or .xls files with PH Section data (RACK No., Size, Balance stock, Rate, Value)
          </p>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-40 flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded p-4 w-full max-w-lg z-50">
          <h3 class="font-semibold mb-2">Create PH Section Product</h3>
...
const visibleProducts = computed(() => {
  // Filter only PH Section products
  let list = products.value.filter(p => p.category === 'PH Section')
...
// Parse PH Section Excel row (array-based: [Rack, Size, Stock, Rate, Value])
const parseASectionExcelRow = (row: any[], idx: number): Product | null => {
  const PH = row?.[0]
  const RIBS = row?.[1]

  // Skip empty rows
  if (!PH && !RIBS) return null
  if (!RIBS) return null
  ...
  return {
    ...
    section: PH !== undefined && PH !== null && PH !== '' ? String(PH) : 'Unknown',
    category: 'PH Section',
    name: `${PH ? String(PH) : 'PH'}-${meter}`,
    size: String(RIBS),
    sku: `${PH ? String(PH) : 'PH'}-${RIBS}`,
    stock,
    rate,
...
// Handle PH Section Excel file upload
const handleASectionExcelUpload = async (event: Event) => {
  ...
  try {
    // Always clear existing PH Section local data and history before processing
    products.value = products.value.filter(p => p.category !== 'PH Section')
    persistProducts()
    ...
    dataRows.forEach((row: any[], idx: number) => {
      ...
      if (!product) {
        ...
        console.warn('PH Section upload skipped row', { idx, row })
        return
      }
      ...
      if (product.stock > 0) {
        addTransaction({
          ...
          timestamp: Date.now(),
          type: 'IN',
          description: `Excel upload: New PH Section product added`,
          stockBefore: 0,
...
const createProduct = () => {
  ...
  const p: Product = { 
    id, 
    category: 'PH Section',
    section: c.section || 'PH',
    name: c.name || `PH-${c.size}`,
    sku: `${c.section || 'PH'}-${c.size}`,
    size: c.size || '',
