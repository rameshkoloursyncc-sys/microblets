    ...
    <table class="w-full text-sm text-left text-gray-600 dark:text-gray-300">
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-40 flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded p-4 w-full max-w-lg z-50">
         ...
         dark:bg-gray-700 dark:text-white dark:border-gray-600" :placeholder="section || 'e.g., A, B, SPA'" />
            </label>
...
const selectedProduct = ref<CoggedBelt | null>(null)
...
const visibleProducts = computed(() => {
  ...
  if (searchTerm.value) {
    ...
    if (searchParts.length >= 2) {
      ...
      // Single search term: match section OR size
      list = list.filter(p => 
...
const startEdit = (product: CoggedBelt, field: keyof CoggedBelt | 'in_qty' | 'out_qty') => { 
...
const showHistory = async (product: CoggedBelt) => {
