        ...
        <div class="mb-2 sm:mb-4 overflow-x-auto">
          <div class="flex gap-2 sm:gap-4 pb-2 min-w-max sm:grid sm:grid-cols-2 lg:grid-cols-4 sm:min-w-0">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-2 sm:p-4 min-w-[140px] sm:min-w-0">
              ...
              <div class="text-sm sm:text-xl font-bold text-red-600 dark:text-red-400">{{ outOfStockCount }}</div>
            </div>
          </div>
        </div>
      ...
      <thead class="bg-gray-50 dark:bg-gray-700 text-xs uppercase sticky top-0 z-20">
        <tr>
         <th class="py-2 sm:py-3 px-1 sm:px-3">Section</th>
                ...
                <th class="py-2 sm:py-3 px-1 sm:px-3">{{ props.section?.startsWith('NEOPRENE') ? 'FULL SLEEVE' : 'FULL SLEEVE' }}</th>
                <th class="py-2 sm:py-3 px-1 sm:px-3 text-center">MM Sleeve</th>
                <th class="py-2 sm:py-3 px-1 sm:px-3 text-right">Total Value</th>
                <th class="py-2 sm:py-3 px-1 sm:px-3 hidden sm:table-cell">Remark</th>
                <th class="py-2 sm:py-3 px-1 sm:px-3 text-center">IN/OUT</th>
          ...
          <tr v-for="p in visibleProducts" :key="p?.id || 'unknown'" class="border-t hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <td class="py-1 sm:py-2 px-1 sm:px-3">
                  <div v-if="editingCell === `${p?.id}-type`">
                    <input v-model="editValue" @keyup.enter="saveCell(p, 'type')" @keyup.esc="cancelEdit" class="w-16 sm:w-20 p-1 border rounded text-center text-xs" />
                  </div>
                  <div v-else @click="startEdit(p, 'type')" class="cursor-pointer font-bold text-black dark:text-white text-center text-xs sm:text-sm">
                    {{ p?.type || '' }}
                  </div>
                </td>
                ...
                <td class="py-1 sm:py-2 px-1 sm:px-3 text-center">
                  <div v-if="editingCell === `${p?.id}-total_mm`">
                    <input v-model.number="editValue" type="number" step="0.01" min="0"  @keyup.enter="saveCell(p, 'total_mm')" @keyup.esc="cancelEdit" class="w-16 sm:w-20 p-1 border rounded text-center text-xs" />
                  </div>
                  <div v-else @click="startEdit(p, 'total_mm')" class="cursor-pointer">
                    <span :class="getStockClass(p)" class="font-medium text-xs sm:text-sm">{{ Number(p?.total_mm || 0).toFixed(2) }}mm</span>
                  </div>
                </td>

                <!-- Value -->
                <td class="py-1 sm:py-2 px-1 sm:px-3 text-right font-medium text-green-600 text-xs">₹{{ Number(p?.value || 0).toFixed(2) }}</td>
                  ...
                  <div class="flex items-center justify-center gap-1">
                    ...
                    <div class="flex flex-col gap-1">
                      <div class="flex gap-1">
                        <button @click="showInOutModal(p, 'IN', 'total_mm')" class="px-1 sm:px-2 py-1 text-xs bg-green-600 text-white rounded hover:bg-green-700">
                          IN
                        </button>
                        <button @click="showInOutModal(p, 'OUT', 'total_mm')" class="px-1 sm:px-2 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700">
                          OUT
                        </button>
                      </div>
                    </div>
