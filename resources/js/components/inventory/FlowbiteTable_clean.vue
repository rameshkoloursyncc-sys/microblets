...
// Use section-specific storage keys to prevent data mixing
const getSectionName = () => {
  if (props.initialCategories && props.initialCategories.length === 1) {
    return props.initialCategories[0].replace(' Section', '')
  }
  return 'all'
}

const STORAGE_KEY = computed(() => `microbelts_products_${getSectionName()}_v1`)
const HISTORY_KEY = computed(() => `inventory_history_${getSectionName()}`)
...
const loadProducts = async () => {
  ...
  try {
    // Try to load from section-specific JSON file FIRST
    if (props.initialCategories && props.initialCategories.length === 1) {
      const sectionName = props.initialCategories[0].replace(' Section', '')
    ...
    // For non-section-specific views, check localStorage
    const raw = localStorage.getItem(STORAGE_KEY.value)
    if (raw) {
      try {
...
const visibleProducts = computed(() => {
  ...
  // Debug logging
  console.log('=== visibleProducts Debug ===')
  console.log('Total products:', list.length)
  console.log('Props initialCategories:', props.initialCategories)
  ...
  if (props.initialCategories && props.initialCategories.length > 0) {
    ...
    console.log(`Filtered by initialCategories: ${beforeFilter} -> ${list.length}`)
    
    // Show which products match
    list.forEach(p => {
      console.log(`Matched product: ${p.name} (${p.category})`)
    })
  } else if (selectedCategory.value) {
    const beforeFilter = list.length
    list = list.filter(p => p.category === selectedCategory.value)
    console.log(`Filtered by selectedCategory: ${beforeFilter} -> ${list.length}`)
  }
  ...
  try {
    ...
    uploadedData.forEach((item, index) => {
      ...
      const newProduct: Product = {
        id: uniqueId,
        name: item.name || (item.section ? String(item.section).toUpperCase() : ''),
        sku: `${item.section.toUpperCase()}-${item.size}`,
        size: String(item.size),
        stock: Number(item.balanceStock) || 0,
        reorder_level: Math.max(1, Number(item.reorder_level) || 5),
        rate: Number(item.rate) || 0,
        value: Number(item.value) || (Number(item.balanceStock) * Number(item.rate)),
        category: item.category || mapSectionToCategory(item.section),
        in_qty: 0, // Reset for new entries
        out_qty: 0, // Reset for new entries
        items_per_sleve: Number(item.items_per_sleve) || 1
      }
      ...
      if (!newProduct.name || newProduct.size === '0' || newProduct.size === '') {
...
const downloadJson = () => {
  ...
  // Export with full structure like SPA
