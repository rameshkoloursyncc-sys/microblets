   ...
   <tr v-for="p in visibleProducts" :key="p.id" class="border-t hover:bg-gray-50 dark:hover:bg-gray-700">
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-40 flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded p-4 w-full max-w-lg z-50">
         ...
         dark:bg-gray-700 dark:text-white dark:border-gray-600" :placeholder="section || 'e.g., TS8M'" />
            </label>

            <label>Width
              <input v-model="createForm.width" class="w-full p-2 border rounded
         bg-white text-gray-900 border-gray-300
         dark:bg-gray-700 dark:text-white dark:border-gray-600" placeholder="e.g., 150" />
            </label>

            <label>Meter (Inventory Quantity)
              <input v-model.number="createForm.meter" type="number" step="0.01" class="w-full p-2 border rounded
         bg-white text-gray-900 border-gray-300
         dark:bg-gray-700 dark:text-white dark:border-gray-600" min="0" placeholder="Meter quantity in stock" />
            </label>

            <label>Rate
              <input v-model.number="createForm.rate" type="number" step="0.01" class="w-full p-2 border rounded
         bg-white text-gray-900 border-gray-300
         dark:bg-gray-700 dark:text-white dark:border-gray-600" placeholder="Rate per unit" />
            </label>
