...
export function useVeeBelts(section?: string) {
  const products = ref<VeeBelt[]>([])
  const loading = ref(false)
  const error = ref<string | null>(null)

  console.log('useVeeBelts initialized with section:', section)
  ...
  // Create product
  const createProduct = async (data: Partial<VeeBelt>) => {
    loading.value = true
    error.value = null
    try {
      const response = await axios.post('/api/vee-belts', data)
  ...
  // Update product
  const updateProduct = async (id: number, data: Partial<VeeBelt>) => {
    loading.value = true
    error.value = null
    try {
      const response = await axios.put(`/api/vee-belts/${id}`, data)
      await fetchProducts() // Refresh list
      return response.data
    } catch (err: any) {
  ...
  const deleteProduct = async (id: number) => {
    ...
    error.value = null
    try {
      await axios.delete(`/api/vee-belts/${id}`)
      await fetchProducts() // Refresh list
    } catch (err: any) {
  ...
  // Bulk import
  const bulkImport = async (productsData: Partial<VeeBelt>[], mode: 'append' | 'replace') => {
    loading.value = true
    error.value = null
    try {
      const response = await axios.post('/api/vee-belts/bulk-import', {
  ...
  const inOutOperation = async (productIds: number[], type: 'IN' | 'OUT', quantity: number) => {
    ...
    error.value = null
    try {
      const response = await axios.post('/api/vee-belts/in-out', {
  ...
  const getTransactions = async (productId: number): Promise<Transaction[]> => {
    try {
      const response = await axios.get(`/api/vee-belts/${productId}/transactions`)
