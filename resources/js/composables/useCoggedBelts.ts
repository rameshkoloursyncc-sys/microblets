...
export function useCoggedBelts(section?: string) {
  const products = ref<CoggedBelt[]>([])
  const loading = ref(false)
  const error = ref<string | null>(null)

  console.log('useCoggedBelts initialized with section:', section)
  ...
  const fetchProducts = async () => {
    ...
    try {
      const url = section 
        ? `/api/cogged-belts/section/${section}`
        : '/api/cogged-belts?paginate=false'
  ...
  // Create product
  const createProduct = async (data: Partial<CoggedBelt>) => {
    loading.value = true
    error.value = null
    try {
      const response = await axios.post('/api/cogged-belts', data)
  ...
  // Update product
  const updateProduct = async (id: number, data: Partial<CoggedBelt>) => {
    loading.value = true
    error.value = null
    try {
      const response = await axios.put(`/api/cogged-belts/${id}`, data)
      await fetchProducts() // Refresh list
      return response.data
    } catch (err: any) {
  ...
  const deleteProduct = async (id: number) => {
    ...
    error.value = null
    try {
      await axios.delete(`/api/cogged-belts/${id}`)
      await fetchProducts() // Refresh list
    } catch (err: any) {
  ...
  // Bulk import
  const bulkImport = async (productsData: Partial<CoggedBelt>[], mode: 'append' | 'replace') => {
    loading.value = true
    error.value = null
    try {
      const response = await axios.post('/api/cogged-belts/bulk-import', {
  ...
  const inOutOperation = async (productIds: number[], type: 'IN' | 'OUT', quantity: number) => {
    ...
    error.value = null
    try {
      const response = await axios.post('/api/cogged-belts/in-out', {
  ...
  const getTransactions = async (productId: number): Promise<Transaction[]> => {
    try {
      const response = await axios.get(`/api/cogged-belts/${productId}/transactions`)
